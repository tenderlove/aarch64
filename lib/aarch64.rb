...
module AArch64
  ...
  class Assembler
    ...
    def adds d, n, m, option = nil, extend: nil, amount: 0, lsl: 0, shift: :lsl
      ...
      if extend
        extend = Utils.sub_decode_extend32(extend)
        a ADDS::ADDSUB_ext.new(d, n, m, extend, amount, d.sf)
      else
        if m.integer?
          a ADDS::ADDSUB_imm.new(d, n, m, lsl / 12, d.sf)
        else
          shift = [:lsl, :lsr, :asr].index(shift) || raise(NotImplementedError)
          a ADDS::ADDSUB_shift.new(d, n, m, shift, amount, d.sf)
        end
      end
    end
    ...
    def and d, n, m, shift: :lsl, amount: 0
      if m.integer?
        ...
        a AND::LOG_imm.new(d, n, enc.immr, enc.imms, enc.n, d.sf)
      else
        shift = [:lsl, :lsr, :asr, :ror].index(shift) || raise(NotImplementedError)
        a AND::LOG_shift.new(d, n, m, shift, amount, d.sf)
      end
    end
    ...
    def ands d, n, m, shift: :lsl, amount: 0
      if m.integer?
        enc = Utils.encode_mask(m, d.size) || raise("Can't encode mask #{m}")
        a ANDS::LOG_imm.new(d, n, enc.immr, enc.imms, enc.n, d.sf)
      else
        shift = [:lsl, :lsr, :asr, :ror].index(shift) || raise(NotImplementedError)
        a ANDS::LOG_shift.new(d, n, m, shift, amount, d.sf)
      end
    end
    ...
    def subs d, n, m, option = nil, extend: nil, amount: 0, lsl: 0, shift: :lsl
      ...
      if extend
        ...
        a SUBS::ADDSUB_ext.new(d, n, m, extend, amount, d.sf)
      else
        if m.integer?
          a SUBS::ADDSUB_imm.new(d, n, m, lsl / 12, d.sf)
        else
          shift = [:lsl, :lsr, :asr].index(shift) || raise(NotImplementedError)
          a SUBS::ADDSUB_shift.new(d, n, m, shift, amount, d.sf)
        end
      end
    end
